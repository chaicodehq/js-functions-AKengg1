...
export function calculateDosaOrder(type, quantity = 1, isSpicy = false) {
  if (typeof type !== "string" || Number.isNaN(quantity) || quantity <= 0)
    return null;
  else {
    const dosa = ["plain", "masala", "onion", "butter", "paper", "cheese"];
    const price = [40, 60, 50, 70, 90, 80];
    if (!dosa.includes(type)) return null;
    let pricePerDosa = price[dosa.indexOf(type)];
